...
| 8 | **Build in sprints.** Per sprint: FREEZE.md (Law 10) → ignition key with ground-truth questions (Law 5) → Hands session → branch → preview → harness → Tower certification against raw → Commander's eye → merge (Hands open the PR and wait for green; the merge button is always the Commander's — Claude Code auto mode refuses merges to main regardless of chat approval). Small fixes direct; taste, scope, architecture through the Tower. | Merged PRs with green harness; LEDGER entry per sprint. | Tower briefs, Hands build, Commander merges |
